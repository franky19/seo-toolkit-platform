...
export class SEOAnalyzer {
  static analyzeSEO($: CheerioAPI, url: string): SEOAuditResult {
    ...
    const scores = [
      ...
      technical.status === 'PASS' ? 100 : technical.status === 'WARNING' ? 60 : 20,
      social.status === 'PASS' ? 100 : social.status === 'WARNING' ? 60 : 20,
      indexability.status === 'PASS' ? 100 : indexability.status === 'WARNING' ? 60 : 20,
    ];
  ...
  private static analyzeMeta($: CheerioAPI): SEOAuditResult['meta'] {
    ...
    return {
      status,
      data: { title, description, canonical, robots, viewport },
      issues,
    };
  }

  private static analyzeTechnical($: CheerioAPI, url: string): SEOAuditResult['technical'] {
    const issues: string[] = [];
  ...
  private static analyzeIndexability($: CheerioAPI): SEOAuditResult['indexability'] {
  ...
  static analyzeSchema($: CheerioAPI): SchemaValidationResult {
    ...
    const status: AuditStatus = schemas.length === 0 ? 'ERROR' : issues.length > 2 ? 'WARNING' : 'PASS';
  ...
  static async analyzeSitemap(baseUrl: string): Promise<SitemapValidationResult> {
    const sitemapUrls = [
      '/sitemap.xml',
      '/sitemap_index.xml',
      '/news-sitemap.xml',
  ...
  static analyzeAISearch($: CheerioAPI, schemaResult: SchemaValidationResult): AISearchAuditResult {
    const issues: string[] = [];
    ...
    const schemaCompleteness = Math.round((presentSchemas / totalSchemas) * 100);
    ...
    const status: AuditStatus = schemaCompleteness >= 80 ? 'PASS' : schemaCompleteness >= 50 ? 'WARNING' : 'ERROR';
    const score = Math.round(schemaCompleteness * 0.6 + (issues.length === 0 ? 40 : Math.max(0, 40 - issues.length * 10)));
